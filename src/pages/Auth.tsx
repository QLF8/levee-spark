          ...
          <div className="text-white space-y-8 hidden md:block animate-fade-in">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full border border-white/20">
                ...
                <span className="text-sm font-medium">Plateforme de financement innovante</span>
              </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {[
                { icon: Rocket, title: "Levée rapide", desc: "Présentez votre projet en quelques clics" },
                { icon: Users, title: "Réseau qualifié", desc: "Accédez à des milliers d'investisseurs actifs" },
                { icon: TrendingUp, title: "Succès garanti", desc: "75% de taux de matching réussi" }
              ].map((item, i) => (
                <div 
                  key={i} 
                  className="flex items-start gap-4 bg-white/5 backdrop-blur-sm p-4 rounded-xl border border-white/10 hover:bg-white/10 transition-all duration-300 hover:scale-105"
                >
            ...
            <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 md:p-10">
              ...
              <Tabs value={isLogin ? "login" : "signup"} onValueChange={(v) => setIsLogin(v === "login")}>
                <TabsList className="grid w-full grid-cols-2 mb-8 bg-gray-100 p-1">
                  <TabsTrigger 
                    value="login" 
                    className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#004aad] data-[state=active]:to-[#0066cc] data-[state=active]:text-white transition-all duration-300"
                  >
                    Connexion
                  </TabsTrigger>
                  <TabsTrigger 
                    value="signup" 
                    className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#FF6B35] data-[state=active]:to-[#FF8C5A] data-[state=active]:text-white transition-all duration-300"
                  >
                    Inscription
                  </TabsTrigger>
                </TabsList>
                  ...
                  <form onSubmit={handleLogin} className="space-y-5">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className="h-12 border-gray-300 focus:border-[#004aad] focus:ring-[#004aad] transition-all duration-300"
                        required
                      />
                    </div>
                    <div className="space-y-2">
                    ...
                    <Button 
                      type="submit" 
                      className="w-full h-12 bg-gradient-to-r from-[#004aad] to-[#0066cc] hover:from-[#003d8f] hover:to-[#0052a8] text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02]" 
                      disabled={loading}
                    >
                ...
                <TabsContent value="signup" className="animate-fade-in">
                  <form onSubmit={handleSignup} className="space-y-4">
                    ...
                    <Button 
                      type="submit" 
                      className="w-full h-12 bg-gradient-to-r from-[#FF6B35] to-[#FF8C5A] hover:from-[#FF5722] hover:to-[#FF7043] text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02]" 
                      disabled={loading}
                    >
                      {loading && <Loader2 className="mr-2 h-5 w-5 animate-spin" />}
                      Créer mon compte
